...
- 偶现 `libdjisdk_jni.so` 崩溃：**已修复**
- 在 Matrice 300 RTK 飞行器上，航线状态回调异常：**已修复**
- 在 Matrice 300 RTK 飞行器上，调用`KeyCameraMode`监听失效：**已修复**
- 在 Mavic 3E 飞行器上，调用`KeyExposureModeRange`获取到的范围与 Pilot 不一致：**已修复**
- 在 Mini 3 Pro 飞行器上，调用`isVirtualStickEnable` 获取的虚拟摇杆状态异常：**已修复**
