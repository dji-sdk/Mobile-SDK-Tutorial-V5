...
- **`WaylineExecutingInfoListener` has no callback when executing waypoint mission on M300 RTK:** Fixed
...
- **`WaypointMissionExecuteStateListener` does not call back `FINISHED` after the waypoint mission is finished on M300 RTK:** Fixed
...
### Bug Fixes
- **The aircraft attitude is unstable when executing waypoint mission on M300 RTK:** fixed
